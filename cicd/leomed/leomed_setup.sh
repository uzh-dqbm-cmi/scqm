...
alias sing_cecile_gpu='srun --cpus-per-task 2 --mem-per-cpu 13G --time 24:00:00 -p gpu --gpus-per-node=1 --pty singularity shell --nv -H $HOME -B /cluster/work/medinfmk/scqm:/opt /cluster/work/medinfmk/scqm/containers/scqm.img'
# jupyter notebook
alias jup_cecile='jupyter notebook --no-browser --ip=$(hostname -i) --port 6070'
# gpu job
alias gpu_job='srun --job-name gpu_inter --cpus-per-task 3 --mem-per-cpu 15G --time 24:00:00 -p gpu --gres=gpu:rtx1080ti:1 --pty bash'
