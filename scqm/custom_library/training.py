import torch
import numpy as np
from scqm.custom_library.utils import MulticlassMetrics, Metrics
...
class AutoEncoderAdaptivenetTrainer(Trainer):
    def __init__(
        ...
        if model.task == "regression":
            ...
            self.weights = self.get_weights(self.dataset)
            # self.criterion = torch.nn.BCEWithLogitsLoss(reduction='sum', pos_weight=self.weights)
            self.pred_criterion = torch.nn.CrossEntropyLoss(
                reduction="sum", weight=self.weights
            )
    ...
    def train_model(self, model, partition, debug_patient):
        ...
        while (self.current_epoch < self.n_epochs) and self.early_stopping == False:
            ...
            batch.get_masks(self.dataset, debug_patient)
            self.update_epoch_and_indices(batch)
            self.decod_loss, self.pred_loss = model.apply_and_get_loss(
                self.dataset, self.dec_criterion, self.pred_criterion, batch
            )
            ...
            if len(batch.available_indices) == len(batch.all_indices):
                with torch.no_grad():
                    self.loss_per_epoch[self.current_epoch - 1] = (
                        self.decod_loss + self.pred_loss
                    )
                    ...
                    #     metrics_val = Metrics(device=model.device)
                    (
                        self.decod_loss_valid,
                        self.pred_loss_valid,
                    ...
                    self.loss_per_epoch_valid[self.current_epoch - 1] = (
                        self.decod_loss_valid + self.pred_loss_valid
                    )
                    self.loss_per_epoch_decod_valid[
                        self.current_epoch - 1
                    ] = self.decod_loss_valid
                    self.loss_per_epoch_pred_valid[
                        self.current_epoch - 1
                    ] = self.pred_loss_valid
                    # metrics.get_metrics()
                    # f1 = metrics.returned_metric
                    ...
                    # re-initialize metrics for new epoch
                    print(
                        f"epoch : {self.current_epoch} pred loss {self.pred_loss} loss_valid {self.pred_loss_valid}"
                    )
                    print(
                        f"epoch : {self.current_epoch} decod loss {self.decod_loss} loss_valid {self.decod_loss_valid}"
                    )
                    # if model.task == 'classification':
                    #     metrics = MulticlassMetrics(
                    #         device=model.device, possible_classes=torch.tensor([0, 1, 2], device=model.device))
    ...
    def plot_other_losses(self):
        plt.figure()
        plt.plot(
            range(0, len(self.loss_per_epoch_decod[: self.current_epoch]), 1),
            self.loss_per_epoch_decod[: self.current_epoch],
        )
        plt.plot(
            range(0, len(self.loss_per_epoch_decod[: self.current_epoch]), 1),
        ...
        plt.figure()
        plt.plot(
            range(0, len(self.loss_per_epoch_pred[: self.current_epoch]), 1),
            self.loss_per_epoch_pred[: self.current_epoch],
        )
        plt.plot(
            range(0, len(self.loss_per_epoch_pred[: self.current_epoch]), 1),
