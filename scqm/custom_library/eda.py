...
def find_regex_columns(df_dict, regex):
    ...
    columns_dict = {
        column: [index for index, table in df_dict.items() if column in table.columns]
...
def link_patients_visits(df_dict):
    ...
    patients_visits = {
        elem: list(grouped_visit_df.get_group(elem)["uid_num"].values)
        if elem in df_dict["visits"]["patient_id"].unique()
        else []
        for elem in df_dict["patients"]["patient_id"].unique()
    }
    ...
    ].apply(get_interval, axis=1)
    grouped_med_df = df_dict["medications"].groupby("patient_id")
    num_of_medications = grouped_med_df.apply(len)
    print(
        f"Min number of medications {num_of_medications.min()}, max num of medications {num_of_medications.max()} mean number of medications {num_of_medications.mean()}"
    )

    # patient dict with visits and medications
    patient_dict = {
        patient: {
            "visit_times": grouped_visit_df.get_group(patient)["visit_date"].values
            if patient in df_dict["visits"]["patient_id"].unique()
            else [],
            "visit_id": grouped_visit_df.get_group(patient)["uid_num"].values
            if patient in df_dict["visits"]["patient_id"].unique()
            else [],
            "medication_intervals": grouped_med_df.get_group(patient)[
                "start_end"
            ].values
            if patient in df_dict["medications"]["patient_id"].unique()
            else [],
        }
        for patient in df_dict["patients"]["patient_id"].unique()
    }
...
def eda_adanet_feature(gen_df, md_df, v_df):
    ...
    visits_df = v_df.copy()
    general_df["age"] = (
        pd.to_datetime("today") - general_df["date_of_birth"]
    ) / np.timedelta64(1, "Y")
    general_df["disease_duration"] = (
        pd.to_datetime("today") - general_df["date_first_symptoms"]
    ) / np.timedelta64(1, "Y")
    general_df.drop(
        columns=[
            "date_of_birth",
    ...
    med_df.drop(
        columns=["patient_id", "medication_start_date", "medication_end_date"],
        inplace=True,
    )
    ...
    categories = pd.concat(
        {k: pd.DataFrame(v, index=[0]).T for k, v in categories.items()}, axis=0
    )
