...
def plot_patient_predictions(
    ...
    if plot_time:
        ...
        targets = df[df.patient_id == p].targets
        predictions = df[df.patient_id == p].predictions
        plt.plot(range(1, len(targets) + 1), targets, "--o", label="true values", c='blue')
        plt.plot(range(1, len(targets) + 1), predictions, "--o", label="predictions", c='darkgreen')
        plt.xticks(range(1, len(targets) + 1))
...
#     plt.xlabel('target value')
    ...
    (freq, bins, _) = plt.hist(x=df['targets'], bins=n_bins, color='#0504aa',
                               alpha=0.5, rwidth=0.85, label='targets')
    plt.xlabel('value', fontsize=14)
    plt.ylabel('frequency', fontsize=14)
    plt.title('Frequency of target label', fontsize=14)
    indices_sorted = np.argsort(freq)
    bins_sorted = [(bins[index - 1], bins[index]) if index > 0 else (0, bins[index]) for index in indices_sorted]
    freq_sorted = [freq[index] for index in indices_sorted]
    tmp['bin'] = np.nan
    tmp['freq_bin'] = np.nan
    for elem in range(len(tmp)):
        for index, bin_ in enumerate(bins_sorted):
            if (bin_[0] <= tmp['targets'].iloc[elem]) & (tmp['targets'].iloc[elem] < bin_[1]):
                tmp['bin'].iloc[elem] = index
                tmp['freq_bin'].iloc[elem] = freq_sorted[index]
    plt.figure()
    plt.grid(visible=True)
    plt.plot(tmp.groupby('freq_bin').apply(lambda x: sum((x.targets - x.predictions)**2) / len(x)), '--o')
