...
class XgbBaselineResults(Results):
    def evaluate_model(self, subset, target_name, return_vectors=False):
        ...
        pred = self.model.predict(xgb.DMatrix(test_tensor), iteration_range=(0, self.model.best_iteration + 1))
        # rescale
        pred_rescaled = torch.tensor(pred * (max_ - min_) + min_)
