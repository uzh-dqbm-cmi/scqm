...
from preprocessing import extract_other_features, load_dfs_all_data, preprocessing, extract_adanet_features
...
if __name__ == '__main__':
    ...
    if model == 'adanet':
        ...
        df_dict_pro = {'a_visit': visits_df, 'patients': patients_df, 'med': medications_df, 'targets': targets_df,
                        'socio': socioeco_df, 'radai': radai_df, 'haq': haq_df}
    else:
        events_to_keep = ['a_visit', 'med', 'haq', 'socioeco', 'sf_12', 'euroquol', 'healthissues']
        df_dict_pro, events= extract_other_features(
            df_dict, transform_meds=True, das28=True, only_meds=True, nan_prop=0.5, events_to_keep=events_to_keep)
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    min_num_visits = 2
    if model == 'adanet':
        dataset = Dataset(device, df_dict_pro, df_dict_pro['patients']['patient_id'].unique(
        ), 'das28_increase', ['a_visit', 'med', 'socio', 'radai', 'haq'], min_num_visits)
        dataset.inclusion_criteria()
    else:
        dataset = Dataset(device, df_dict_pro, df_dict_pro['patients']['patient_id'].unique(
        ), 'das28_increase', events, min_num_visits)
        dataset.inclusion_criteria()
    ...
    if model == 'adanet':
        ...
        with open('/opt/tmp/saved_dataset_inclusion_more_less_bis.pickle', 'wb') as handle:
            pickle.dump(dataset, handle)
    dataset.create_dfs()
    if model == 'adanet':
