...
def create_dataset_baselines(df_dict):
    ...
    (
        ...
        targets_df,
        joint_df,
    ) = extract_adanet_features(df_dict, das28=True, joint_df=True)
    df_dict_baselines = {
        "visits": visits_df,
    ...
    dataset = Dataset(
        df_dict_baselines, df_dict_baselines["patients"]["patient_id"].unique()
    )
...
def train_model(
    ...
    # validation
    feature_tensor_val = (
        dataset.joint_df_scaled_tensor_valid.detach().clone().to(device)
    )
    patient_tensor_val = (
        dataset.patients_df_scaled_tensor_valid.detach().clone().to(device)
    )
    input_tensor_valid, targets_valid, _ = get_input_tensors(
        device,
        dataset.valid_ids,
        "valid",
        feature_df,
...
def get_batch(
    ...
    input_tensor, input_targets, _ = get_input_tensors(
        device,
        batch,
        "train",
        feature_df,
...
def get_input_tensors(
    ...
    hist_size,
    debug_patient,
    method="predict_all_visits",
):
    # TODO make this more robust
    visit_flag = [
        col
        for col in feature_df.columns
        if col not in ["patient_id", "uid_num", "med_id"]
    ].index("is_visit")
    target_flag = [
        col
        for col in feature_df.columns
        if col not in ["patient_id", "uid_num", "med_id"]
    ].index("das283bsr_score")
    visit_date_flag = [
        col
        for col in feature_df.columns
        if col not in ["patient_id", "uid_num", "med_id"]
    ...
    for index_in_subset, elem in enumerate(subset):
        if subset_name == "train":
            feature_indices = feature_df[feature_df.patient_id == elem][
                "tensor_indices_train"
            ].values
            general_indices = patient_df[patient_df.patient_id == elem][
                "tensor_indices_train"
            ].values
        elif subset_name == "valid":
            feature_indices = feature_df[feature_df.patient_id == elem][
                "tensor_indices_valid"
            ].values
            general_indices = patient_df[patient_df.patient_id == elem][
                "tensor_indices_valid"
            ].values
        ...
        # transform
        indices_of_visits = torch.tensor(
            [elem for elem in range(len(t)) if t[elem, visit_flag] == 1]
        )
        ...
        if indices_of_visits[flag] < hist_size + 1:
            ...
            t = torch.cat(
                [
                    torch.full(
                        size=(hist_size + 1 - indices_of_visits[flag], t.shape[1]),
                        fill_value=-1,
                        device=device,
                    ),
                    t,
...
def train_and_test_pipeline_baseline(
    dataset, batch_size, n_epochs, hist_size, debug_patient=False
):
    # device
    ...
    with torch.no_grad():
        ...
        patient_df = dataset.patients_df_proc.copy()
        feature_tensor_test = (
            dataset.joint_df_scaled_tensor_test.detach().clone().to(device)
        )
        patient_tensor_test = (
            dataset.patients_df_scaled_tensor_test.detach().clone().to(device)
        )
        # subset = ['0257c010-933a-1a73-3b48-46a2c6b254e8', '07367bce-2706-3fe9-acc5-800560f87048']
        input_tensor_test, targets_test, num_targets_per_patient = get_input_tensors(
            device,
            dataset.test_ids,
            "test",
            feature_df,
        ...
        results_df = create_results_df(
            ...
            out_test,
            algo="baseline",
            num_targets=num_targets_per_patient,
        )
    ...
    if len(loss_per_epoch_valid) > 0:
        ...
        plt.plot(
            range(
                0,
                len(loss_per_epoch),
                int(len(loss_per_epoch) / len(loss_per_epoch_valid)),
            ),
